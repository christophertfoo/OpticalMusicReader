...
public class HalfNoteDetection extends NoteHeadDetection{
	...
	public void makeNoteHeadTemplate() {
		...
		for(int y=0; y > (2*height*-1);y--){
			for(int x=0; x < (2*height); x++){
				double value = (Math.pow((x-height)*Math.cos(angleRotation) + (y+height)*Math.sin(angleRotation), 2)/Math.pow(height,2)) + 
						(Math.pow((x-height)*Math.sin(angleRotation) - (y+height)*Math.cos(angleRotation),2)/Math.pow(height/2.0, 2));
