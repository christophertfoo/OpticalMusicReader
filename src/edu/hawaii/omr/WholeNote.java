...
public class WholeNote extends NoteHead {
	...
	public Mat makeNoteHeadTemplate() {
		...
		for(int y=0; y>(2*height*-1); y--){
			for(int x=0; x< (2*height); x++){
				double value = (Math.pow((x - height) * Math.cos(holeAngleRotation)
						+ (y + height) * Math.sin(holeAngleRotation), 2) / Math
							.pow(height/2.0, 2))
						+ (Math.pow((x - height) * Math.sin(holeAngleRotation)
								- (y + height) * Math.cos(holeAngleRotation), 2) / Math
									.pow(height / 3.0, 2));
				if (value < 1) {
					double data[] = { 255 };
